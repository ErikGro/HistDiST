...
from util.kid_score import getKIDscore
import timm
...
#                 tensor_batch = torch.stack([transform(image) for image in batch]).to("cuda")
...
#     features_ihc_ground_truth = torch.load("/graphics/scratch2/students/grosskop/AdaptiveSupervisedPatchNCE/features_valB.pt").cpu()
...
# def compute_metrics(pred_dir: str) -> Metrics:
#     start = time.time()
#     targ_dir = '/graphics/scratch2/students/grosskop/benchmark_er_testset/valB'
...
#     compute_features(pred_dir)
#     acc, accTop3 = compute_accuracy(feature_file_name(targ_dir), feature_file_name(pred_dir), "cosine")
...
#         real = io.imread(os.path.join(targ_dir, i + ".jpg"))#img_format))

#         fake = to_tensor(fake).to("cuda")
#         real = to_tensor(real).to("cuda")
...
#     real_paths = [os.path.join(targ_dir, f + ".jpg") for f in img_list]
#     fake_paths = [os.path.join(pred_dir, f + img_format) for f in img_list]
...
#     m1, s1 = calculate_activation_statistics(real_paths, model, batch_size=10, dims=dims,
#                                         device="cuda", num_workers=num_workers)
...
#     for i in tqdm(img_list):
#         fake = io.imread(os.path.join(pred_dir, i + img_format))
#         real = io.imread(os.path.join(targ_dir, i + ".jpg"))
...
#     print("Compute KID")
#     kidMean = getKIDscore(targ_dir, pred_dir)
    
#     end = time.time()
#     print("Finished after: ", (end - start))

#     return Metrics(average_ssim, average_psnr, fid_value, kidMean, all_phv, acc, accTop3)

# if __name__ == "__main__":
#     metrics = compute_metrics("/graphics/scratch2/students/grosskop/benchmark_er_testset/instructPix2Pix")
#     print(metrics)
